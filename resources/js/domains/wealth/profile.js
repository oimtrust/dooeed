...
export function initInitialWealthProfile() {
    ...
    const render = () => {
        ...
        list.replaceChildren(...categories.map(([category, title, nameLabel]) => {
            const card = document.createElement('div');
            card.className = 'col-md-6 col-xl-4';
            const items = entries.filter((entry) => entry.category === category);
            card.innerHTML = `<div class="card h-100"><div class="card-header"><h2 class="card-title">${title}</h2><button class="btn btn-primary btn-sm ms-auto" data-add="${category}">Tambah</button></div><div class="card-body"><div class="h3">${money(summary?.categories?.[category] ?? 0)}</div><div class="text-secondary small mb-2">${items.length ? '' : 'Belum ada data.'}</div><div class="list-group list-group-flush">${items.map((entry) => `<div class="list-group-item px-0 d-flex align-items-center"><span>${entry.name}<small class="d-block text-secondary">${money(entry.amount)}${entry.debt_type ? ` · ${entry.debt_type === 'other' ? 'Selain kartu kredit & paylater' : 'Kartu kredit & paylater'}` : ''}</small></span><span class="ms-auto"><button class="btn btn-ghost-secondary btn-sm" data-edit="${entry.id}">Ubah</button><button class="btn btn-ghost-danger btn-sm" data-delete="${entry.id}">Hapus</button></span></div>`).join('')}</div></div></div>`;
            return card;
        }));
    };
    ...
    const formFor = (entry = null, category = entry?.category) => {
        ...
        form.addEventListener('submit', async (event) => { event.preventDefault(); const payload = Object.fromEntries(new FormData(form)); payload.category = category; try { entry ? await initialWealthApi.update(entry.id, payload) : await initialWealthApi.store(payload); form.remove(); await load(); } catch (error) { alert(error.response?.data?.message ?? 'Data gagal disimpan.'); } });
